...
namespace Aliencube.XslMapper.FunctionApp.Configurations
{
    ...
    public class AppSettings
    {
        ...
        public AppSettings()
        {
            ...
            JsonFormatter = new JsonMediaTypeFormatter() { SerializerSettings = jsonSerialiserSettings };
            StorageConnectionString = config.GetValue<string>(AzureWebJobsStorageKey);
            Containers = config.Get<ContainerSettings>(ContainersKey);
            EncodeBase64Output = config.GetValue<bool>(EncodeBase64OutputKey);
        }
