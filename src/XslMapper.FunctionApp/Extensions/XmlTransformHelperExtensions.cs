...
namespace Aliencube.XslMapper.FunctionApp.Extensions
{
    ...
    public static class XmlTransformHelperExtensions
    {
        ...
        public static async Task<IXmlTransformHelper> AddParam(this Task<IXmlTransformHelper> helper, string inputXml)
        {
            var instance = await helper.ConfigureAwait(false);
            return await instance.AddParam(inputXml).ConfigureAwait(false);
        }
